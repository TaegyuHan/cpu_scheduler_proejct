...
class SRT(scheduler.AbstractSchedule):
    """
        SRT 스케줄링
    ...
    def _update_ready(self):
        ...
        heapq.heappush(self._ready_queue,
                       (process.remain_service_time, process))
    ...
    def _dispatch(self):
        ...
        service_end_time = self.time + process.rr_cpu_service_time
        # 프로세스가 동작한 시간 넣기
        self._data_frame_list.append(
            [
                "run",
    ...
    def _time_out(self):
        ...
        if not self._cpu: return # CPU에 프로세스가 없으면 PASS
        if self._cpu[0].check_time_out: return # 타임 슬라이스 초과

        process = self._cpu.pop()
        process.stop_time = self.time
        heapq.heappush(self._ready_queue,
                       (process.remain_service_time, process))
